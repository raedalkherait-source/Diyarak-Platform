...
- `Diyarak.Market.Listing` — initial baseline covered by tests for confirmed publishing-role and transaction-intent value sets, `ListingContext` equality and invalid-value rejection, `ListingPrice` known/on-request behavior and negative-price rejection, and `ListingHeadline` assignment, equality, and blank-value rejection.
