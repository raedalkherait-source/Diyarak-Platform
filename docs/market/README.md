...
- `listing-subject-reference-requirements.md` records the confirmed subject-reference and publication-availability requirements, the decisions implemented by ADR-0010 through ADR-0029, and the remaining unresolved Listing operations beyond Draft editing, external-identity provisioning transport, ownership transfer, and post-publication availability behavior.
...
- `Diyarak.Market.Application` owns `IMarketListingRepository`, `IPropertyExistenceChecker`, and `IMarketTransactionRunner`. `CreateListingUseCase` receives a Property identifier and authenticated actor user identifier, verifies Property existence, creates an actor-owned `Draft` Listing, and adds it through the repository. `UpdateListingUseCase` and `PublishListingUseCase` load owner-controlled Listings through the same repository and transaction boundary. Draft edits require the client-supplied Listing version, while publication uses the loaded version; both saves advance the persisted version and reject stale writes with the same stable concurrency conflict.
