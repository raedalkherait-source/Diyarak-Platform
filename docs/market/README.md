...
- `listing-subject-reference-requirements.md` records the confirmed subject-reference and publication-availability requirements, the decisions implemented by ADR-0010 through ADR-0030, and the remaining unresolved Listing operations beyond Draft editing, external-identity provisioning transport, ownership transfer, and post-publication availability behavior.
...
- `PostgreSqlMarketPropertyRepository` inserts newly created Property aggregates through the existing full-state Property record mapping and table.
