...
- `Diyarak.Api` references the Application layer and registers `PublishListingUseCase` for Host composition. ADR-0022 and ADR-0023 define the future publication HTTP and creator-ownership contracts, but the route is not mapped or exposed until an authentication mechanism and claims mapping are accepted and configured.
- Property creation, loading, update, and save workflows beyond existence checks; Listing creation persistence; authentication implementation and claims mapping; administrative overrides and ownership transfer; authoritative migration of any legacy Listing ownership; publication endpoint exposure; explicit transaction, locking, and isolation guarantees; optimistic concurrency; behavior when a referenced subject later becomes unavailable; additional publication-readiness requirements; additional Listing lifecycle states and transitions; transaction-specific commercial terms; and published-Listing editing workflows remain deferred pending concrete requirements.
