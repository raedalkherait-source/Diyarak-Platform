...
- `listing-subject-reference-requirements.md` records the confirmed subject-reference and publication-availability requirements, the decisions implemented by ADR-0010 through ADR-0032, and the remaining unresolved Listing operations beyond creator direct loading and Draft editing, external-identity provisioning transport, ownership transfer, and post-publication availability behavior.
...
- `PostgreSqlMarketListingRepository` implements `IMarketListingRepository` and adds, loads, and conditionally saves Market Listing state through `PlatformDbContext`; persisted Listing `Version` is configured as the EF Core concurrency token and every successful mutation writes the next version.
