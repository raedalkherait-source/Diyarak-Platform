...
- `listing-subject-reference-requirements.md` records the confirmed subject-reference and publication-availability requirements, the decisions implemented by ADR-0010 through ADR-0027, and the remaining unresolved concurrency, draft editing and other Listing operations, external-identity provisioning transport, ownership transfer, and post-publication availability behavior.
...
- `PostgreSqlMarketListingRepository` implements `IMarketListingRepository` and adds, loads, and saves Market Listing state through `PlatformDbContext`.
