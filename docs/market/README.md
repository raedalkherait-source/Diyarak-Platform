...
- `listing-subject-reference-requirements.md` records the confirmed subject-reference and publication-availability requirements, the decisions implemented by ADR-0010 through ADR-0028, and the remaining unresolved draft-edit concurrency, other Listing operations, external-identity provisioning transport, ownership transfer, and post-publication availability behavior.
...
ADR-0021 places Listing loading and saving inside publication orchestration through the Application-owned `IMarketListingRepository` port. ADR-0027 adds the explicit Application-owned transaction boundary: creation and publication persistence-sensitive work runs in one PostgreSQL `ReadCommitted` transaction. ADR-0028 adds optimistic status concurrency for competing publication attempts while leaving Property row locking, stronger isolation, and future draft-edit concurrency undefined.
...
- `Diyarak.Market.Application` owns `IMarketListingRepository`, `IPropertyExistenceChecker`, and `IMarketTransactionRunner`. `CreateListingUseCase` receives a Property identifier and authenticated actor user identifier, verifies Property existence, creates an actor-owned `Draft` Listing, and adds it through the repository. `PublishListingUseCase` validates Listing and actor identifiers, loads the aggregate, returns the same not-found result for a missing or non-owned Listing, verifies the referenced Property only for the owner, calls `Listing.Publish()`, and conditionally saves only when the persisted status is still `Draft`.
...
- `Diyarak.Market.Application.Tests` verifies Listing creation validation, Property-existence enforcement, actor ownership assignment and Draft persistence, plus publication validation, concealed not-found behavior for missing and non-owned Listings without a Property check or save, missing-Property conflict, publication-state conflict, stale-save concurrency conflict, and successful creator-owned publication and saving.
...
- `PostgreSqlMarketListingRepository` implements `IMarketListingRepository` and adds, loads, and conditionally saves Market Listing state through `PlatformDbContext`; persisted Listing status is configured as an EF Core concurrency token.
...
- `Diyarak.Platform.Persistence.PostgreSql.Tests` verifies Property and Listing table mappings, complete domain-record round trips including publisher ownership, found and missing Property checks, Listing repository insertion, loading, expected-status saving, stale competing publication rejection, dependency-injection registration, and ownership migration safety.
- `Diyarak.Api` references the Application layer, registers `CreateListingUseCase` and `PublishListingUseCase`, and conditionally maps both Market Listing routes when ADR-0025 authentication is enabled. `POST /api/market/listings` creates an actor-owned Draft for an existing Property and returns `201 Created`; `POST /api/market/listings/{listingId}/publish` preserves ADR-0023 concealed not-found behavior for non-owners.
- Property creation, loading, update, and save workflows beyond existence checks; identity-provider vendor selection and external-identity provisioning transport; draft editing and other Listing operations; administrative overrides and ownership transfer; authoritative migration of any legacy Listing ownership; Property row locking or stronger-than-`ReadCommitted` isolation guarantees; optimistic concurrency for future draft editing and other Listing mutations; behavior when a referenced subject later becomes unavailable; additional publication-readiness requirements; additional Listing lifecycle states and transitions; transaction-specific commercial terms; and published-Listing editing workflows remain deferred pending concrete requirements. Market Listing endpoints are exposed only when ADR-0025 authentication is explicitly enabled and require a mapped internal user.
