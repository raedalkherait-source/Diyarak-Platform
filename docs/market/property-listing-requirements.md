...
The listing therefore needs an architectural way to identify its published subject without making Platform Core depend on a Market module. ADR-0010 defines that relationship through the sector-agnostic `ListingSubjectReference` contract, ADR-0011 keeps the reference immutable for the lifetime of a Listing, and ADR-0012 assigns supported subject-type validation to the consuming business module. ADR-0017 defines current Property availability as existence, and ADR-0019 defines the Application-owned Property existence check used for publication-time subject availability. Behavior when a referenced subject later becomes unavailable remains unresolved.

## Implemented Property creation boundary

ADR-0030 defines the first Property application and HTTP creation workflow. `POST /api/market/properties` requires the ADR-0025 mapped-user policy, accepts only fields already represented by the current Property aggregate, generates the Property identifier server-side, and inserts the aggregate through an Application-owned repository implemented by PostgreSQL persistence. ADR-0031 adds authenticated direct loading of a known Property identifier through `GET /api/market/properties/{propertyId}`. ADR-0034 defines internal Property management ownership: every newly created Property stores the mapped internal actor as immutable `OwnerUserId`, while legacy rows may remain unowned because no authoritative backfill is available. Direct loading is owner-only and returns the same concealed `404` for missing, non-owned, and legacy-unowned rows. This ownership is a Diyarak authorization concept, not legal title. ADR-0035 adds mapped-user-only `GET /api/market/properties` for the current owner's management collection, filtered by persisted `OwnerUserId` and excluding legacy-unowned rows. ADR-0042 later defines bounded technical pagination for that management collection with `page`/`pageSize`, identifier ordering, and `hasMore`; ADR-0049 adds requestable adjacent-page `Link` metadata without changing the response body, ownership filter, or `no-store` cache policy. Filtering, search, public discovery, and business sorting remain undefined. Property search, removal/archive, ownership transfer/delegation, and lifecycle behavior remain unresolved; ADR-0036 defines owner-only full Property replacement with optimistic concurrency. ADR-0037 requires the current Listing creator to match the referenced Property's management owner; delegated publication authority for agents, companies, administrators, or other non-owner actors remains unresolved.
...
ADR-0032 adds creator-only direct loading of current Listing management state. ADR-0033 adds mapped-user-only collection loading for the authenticated creator's own Listings; ADR-0042 later defines bounded technical pagination with `page`/`pageSize`, identifier ordering, and `hasMore`, and ADR-0049 adds requestable adjacent-page `Link` metadata while preserving mapped-user ownership filtering and `Cache-Control: no-store`. Search, filters, public discovery, and business sorting remain undefined. ADR-0038 defines anonymous direct loading of known `Published` Listings through `/api/market/public/listings/{listingId}` with a reduced public projection; Draft and missing Listings share `404`, and public Property/subject presentation remains undefined. ADR-0039 adds anonymous bounded collection loading through `/api/market/public/listings` using `page`/`pageSize`, deterministic identifier ordering, and `hasMore`, while keeping the same reduced projection. It does not define search ranking, filtering, recommendations, semantic sort choices, or public Property/subject presentation. ADR-0044 and ADR-0045 add opaque ETag revalidation for public detail and collection pages. ADR-0046 explicitly allows those successful anonymous representations to be stored only with mandatory revalidation (`Cache-Control: public, no-cache`) and prevents expected public `400/404` responses from being stored (`Cache-Control: no-store`), without defining a positive freshness lifetime. ADR-0047 adds bodyless `HEAD` parity for those same anonymous public resources while preserving the existing visibility, pagination, ETag, conditional-revalidation, and cache semantics. ADR-0048 adds HTTP `Link` pagination metadata to successful public collection pages using only the already-defined `page`, `pageSize`, and `hasMore` state; it defines requestable `prev`/`next` navigation without adding total-count, `last`, filtering, ranking, or business-sort semantics. ADR-0049 uses the same adjacent-page Link rules for the two authenticated management collections and centralizes the Host formatter without changing their payloads or persistence queries. ADR-0013 defines only the initial Market Listing lifecycle states `Draft` and `Published` and the explicit `Draft` to `Published` transition. ADR-0014 requires `ListingContext`, `ListingHeadline`, and `ListingPrice` before publication while keeping `ListingAvailableFromDate` optional. ADR-0015 restricts changes to these core publication values to the `Draft` state. Additional lifecycle states and transitions, additional publication-readiness rules, published-Listing editing workflows, publication workflow details, company behavior, additional authorization rules, search behavior, persistence mappings, contact workflows, and API contracts must not be invented before their requirements are defined.
