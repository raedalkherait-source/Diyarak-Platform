...
- `IMarketListingRepository` is the Application-owned port used to add and load Market Listings and to conditionally save a Listing against an expected persisted version.
...
ADR-0032 adds creator-only direct Listing loading through `GetListingUseCase`. The read returns current aggregate state to the owner without a transaction or Property re-check and does not define public Listing presentation.
