...
ADR-0030 defines authenticated `POST /api/market/properties` creation for the already modeled Property fields, returning `201 Created` with the server-generated Property identifier. ADR-0031 adds mapped-user-only `GET /api/market/properties/{propertyId}` direct loading, returning the full currently modeled asset without turning it into a public Listing-presentation contract. Authentication gates these Property capabilities without introducing Property ownership.
